...
### Baseline model
We are starting with the null model and then looking at a model with numerical variables, since we only have numerical qualitative (and not numerical quantitative variable) we are going to start with them.
Then, we run step() to get the best model so far using the AIC (we get deviance of 906.63 and AIC 950.63).
We run anova() and we see we cannot reject the null hypothesis and the models are equivalent so it is better for us the small model when we already know it's significant that they are equivalent
```{r}
library(nnet)
...
```{r}
m3 <- multinom(vote ~ egoposition_immigration*f.ostwest + egoposition_immigration*f.gender + egoposition_immigration+f.gender + f.ostwest + f.gender*f.ostwest, data = train) #adding factors to the model
summary(m3)
m4 <- step(m3)
summary(m4)
AIC(m4,m2)
...
*WE HAVE IT ALREADY ABOVE, THIS PART IS REDUNDANT I DON'T KNOW WHY*
```{r}
library(nnet)
set.seed(1310)
t <- sample(1:nrow(df),round(0.66*nrow(df),0)) # rows for training
...
m3 <- multinom(vote ~ egoposition_immigration*f.ostwest + egoposition_immigration+f.ostwest + egoposition_immigration*f.gender + egoposition_immigration+f.gender + f.gender*f.ostwest, data = train) #adding factors
summary(m3)
m4 <- step(m3)
summary(m4)
anova(m4,m3) #models are equivalent
AIC(m4,m3,m2)

```

Model with target variable f.vote only
Now, we are taking f.vote which has 3 levels, each party is classified to 3 orientations: left, center or right.
When we run the null model we already see a better AIC up to this point. Then we are doing the same process as above where we building a model with numerical qualitative variables, finding the best model so far, adding factors to the model and finding the best model so far with the factors. When we have nested models we are checking anova() to do hypothesis test to whether or not the models are equivalent. lastley, we check AIC for all models to see which one has the lowest. mm4 has Deviance = 890.39 AIC = 942.39. When we compare m4 and mm4 we get a better AIC for mm4 and that is when the parties are classified to 3 orientations.
```{r}
set.seed(1310)
t <- sample(1:nrow(df),round(0.66*nrow(df),0)) # rows for training
train <- df[t,]  # working/training set
...
df$f.pos_imm <- factor(df$f.pos_imm)
summary(df)
set.seed(1310)
t <- sample(1:nrow(df),round(0.66*nrow(df),0)) # rows for training
train <- df[t,]  # working/training set
test <- df[-t,]  # testing set
m5 <- multinom(f.vote ~ political_interest+income , data = train)
summary(m5)
m6 <- step(m5) #the null model is the best in such case
summary(m6) 
m7 <- multinom(f.vote ~ f.pos_imm + f.gender + f.ostwest + f.pos_imm*f.gender + f.pos_imm*f.ostwest + f.gender*f.ostwest + f.pos_imm + f.gender + f.ostwest, data = train)
summary(m7)
m8 <- step(m7)
summary(m8)
anova(m7,m8)
...
## Ordinal model proposal is AfD<CDU/CSU<FDP<Gruene<LINKE<SPD
```{r}
set.seed(1310)
t <- sample(1:nrow(df),round(0.66*nrow(df),0)) # rows for training
train <- df[t,]  # working/training set
test <- df[-t,]  # testing set
summary(train)
train$vote <- factor(train$vote,levels = c("AfD","CDU/CSU","FDP","Gruene","LINKE","SPD"),ordered = T) #I SAW IT IN THE INTERNET
library(MASS)
omm0 <- polr(vote ~ 1 , data = train)
summary(omm0)
omm1 <- polr(vote ~ egoposition_immigration+political_interest+income, data = train) #numeric qualitative variables
summary(omm1)
omm2 <- step(omm1)
summary(omm2)
...
omm3 <- polr(vote ~ egoposition_immigration*f.gender + egoposition_immigration*f.ostwest + political_interest*f.ostwest + f.gender*political_interest + egoposition_immigration + political_interest + f.ostwest + f.gender , data = train,Hess = TRUE) #adding factors #WE GET A WARNING MESSAGE HERE WHICH I THINK IT'S ODD, IT DOESNT LET ME GENERATE SUMMARY UNLESS I ADD Hess = TRUE
summary(omm3)
omm4 <- step(omm3)
summary(omm4)
anova(omm4,omm3)
...
## Ordinal model the proposal is center<left<right
```{r}
set.seed(1310)
t <- sample(1:nrow(df),round(0.66*nrow(df),0)) # rows for training
train <- df[t,]  # working/training set
...
train$f.vote_center<-ifelse(train$f.vote=="center", "center","other")
train$f.vote_center<- factor(train$f.vote_center)
summary(train)
hm1 <- glm(f.vote_center ~ egoposition_immigration+political_interest+income, family = "binomial", data = train)
summary(hm1)
hm2 <- step(hm1)
summary(hm2)
hm3 <- glm(f.vote_center ~ egoposition_immigration*f.ostwest + egoposition_immigration+f.ostwest + egoposition_immigration*f.gender+f.gender, family = "binomial", data = train)
summary(hm3)
hm4 <- step(hm3)
summary(hm4)
AIC(hm4,hm3,hm2,hm1)
hm5 <- glm(f.vote ~ egoposition_immigration+political_interest+income, family = "binomial", data = train[train$f.vote_center=="other",])
summary(hm5)
hm6 <- step(hm5)
summary(hm6)
hm7 <- glm(f.vote ~ egoposition_immigration*f.ostwest + egoposition_immigration*f.gender + political_interest*f.ostwest + political_interest*f.gender + egoposition_immigration + political_interest + f.ostwest + f.gender, family = "binomial", data = train[train$f.vote_center == "other", ])
summary(hm7)
hm8 <- step(hm7)
summary(hm8)
AIC(hm8)+AIC(hm4)
...
ttn<- table(predict(mm4),train$f.vote);ttn
100*sum(diag(ttn))/sum(ttn)
tto <- table(predict(om4),train$f.vote);tto
100*sum(diag(tto))/sum(tto)

table( ifelse(predict(hm4,type="response")>0.5,"other","center"),train$f.vote_center)
table(factor(ifelse(predict(hm8,type="response")<0.5,"left","right"),levels=c("left","right")),train$f.vote[train$f.vote != "center"])
...
The log odds of being in from the east of germany will increase by 0.826 fior CDU/CSU, 0.825 for FDP, 0.824for Gruene, 0.07 for LINKE, 0.67 fSPD. The increasce is similar for three of the candidates and then we can see that SPD is following by not that far. Finally LINKE seems the least increasced by people from the east of germany

Being a female we can see different outcomes where there is also an increase of 1.21 for CDU/CSU, 1.30 for FDP, 1.72for Gruene, 1.15 for LINKE and 1.35 for SPD. In this case, all have a similar incease. However, Gruene is the one being the most benefited of that level.

With the egopositional_immigration there are a lot of levels but we can summarize by saying that levels 1 and 2 have an increase around 22 of the log odss in the two first levels. Then the rest of the levels (3-10) are decreascing the log odds with some levels more variable than others but we can see that the majority of values reside between 10-15.
...
```{r}
predict(m4, type="probs", newdata=data.frame(egoposition_immigration=factor(4), f.ostwest = factor("west"), f.gender=factor("male")))
...
om0 <- polr(vote~ 1 , data = train)
summary(om0)
om1 <- polr(vote ~ egoposition_immigration+political_interest+income, data = train) #numerical qualitative variables
summary(om1)
om2 <- step(om1)
summary(om2)
```
...
### Improving the model
```{r}
om3 <- polr(vote ~ egoposition_immigration*f.ostwest + egoposition_immigration+f.ostwest + egoposition_immigration*f.gender + egoposition_immigration+f.gender, data = train) #adding factors
summary(om3)
om4 <- step(om3)
summary(om4)
om4$zeta
...
```{r}
library(effects)
plot(Effect(focal.predictors = c("egoposition_immigration"), om4))
plot(Effect(focal.predictors = c("f.gender"), om4))
...
hm2 <- step(hm1)
summary(hm2)
hm3 <- glm(f.vote_center ~ egoposition_immigration*f.ostwest + egoposition_immigration+f.ostwest + egoposition_immigration*f.gender + egoposition_immigration+f.gender, family = "binomial", data = train)
summary(hm3)
hm4 <- step(hm3)
summary(hm4)
AIC(hm4,hm3,hm2,hm1)
