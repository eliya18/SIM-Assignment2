...
mh4$deviance;mh4$edf;2*nrow(df)-mh4$edf
1-pchisq(mh4$deviance, 2*nrow(df)-mh4$edf)
```
...
#predict(mh4, type="probs", newdata=data.frame(f.pos_imm=factor("mild"),  f.gender=factor("male"),political_interest=factor(2)))
#predict(mo4, type="probs", newdata=data.frame(f.pos_imm=factor("open"), f.gender=factor("male"),political_interest=factor(4)))
#predict(mo4, type="probs", newdata=data.frame(f.pos_imm=factor("restrictive"), f.gender=factor("male"),political_interest=factor(2)))
#predict(mo4, type="probs", newdata=data.frame(f.pos_imm=factor("restrictive"), f.gender=factor("male"),political_interest=factor(0)))
```
...
tt<-table(predict(mh4),train$f.vote_center);tt #Checks that the model i not predicting part times
...
```{r}
tt<-table(predict(mh4, newdata = test),test$f.vote_center);tt #Checks that the model i not predicting part times
100*sum(diag(tt))/sum(tt) # ACCURACY of the model


predicted <- predict(mh4, newdata = test)
actual <- test$f.vote_center
...
- Herachical proposal: (Sill have to decide)
...
In this project we had to iterate for each propousal to try to obtain the best models. The data quality was very good so our main focus was in the modeling part. The best model we found is ...
