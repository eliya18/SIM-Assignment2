...
Detecting multivariate outliers it's not an easy process since all data is factors and we can't apply methods like mahalanobis distance. One solution could be using MCA (factorial method) to detect anomalies.
...
summary(mo0)
summary(train)
mo1 <- polr(fo.vote ~ f.pos_imm+political_interest+income, data = train) #numeric qualitative variables
summary(mo1)
mo2 <- step(mo1)
summary(mo2)
...
summary(mh0)
mh1 <- glm(f.vote_center ~ f.pos_imm+political_interest+income, family = "binomial", data = train)
summary(mh1)
mh2 <- step(mh1)
summary(mh2)
...
mh3 <- glm(f.vote_center ~ f.pos_imm*f.ostwest + f.pos_imm+f.ostwest + f.pos_imm*f.gender + f.pos_imm+f.gender, family = "binomial", data = train)
summary(mh3)
mh4 <- step(mh3)
summary(hm4)
#comparision
...
tt<-table( ifelse(predict(mh4,type="response")>0.5,"other","center"),train$f.vote_center);tt
